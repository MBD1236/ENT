        ...
        <div class="card-body">
            ...
            <form action="" wire:submit.prevent='update' class="vstack gap-2" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-12 mt-2">
                        <div class="form-floating">
                            <select wire:model="session_id" id="session_id"
                                class="form-control @error('session_id') is-invalid @enderror">
                                <option value="0">Sélectionner une session</option>
                                @foreach($sessions as $session)
                                <option @selected($inscription->session_id == $session->id) value="{{ $session->id }}"
                                    wire:key='{{ $session->id }}'>{{ $session->session}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-floating">
                            <select wire:model="programme_id" id="programme_id"
                                ...
                                <option @selected($inscription->programme_id == $programme->id)
                                    value="{{$programme->id}}" wire:key='{{ $programme->id }}'>{{ $programme->nom}}
                                </option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback">@error('programme_id') {{ $message }} @enderror</div>
