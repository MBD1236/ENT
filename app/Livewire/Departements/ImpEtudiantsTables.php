...
class ImpEtudiantsTables extends Component
{
    ...
    {
         // 1- pour un debut je retourne la liste des etudiants inscrits au departement IMP
         $inscription = Inscription::query()->orderBy("created_at","desc");
         $inscription->orWhereHas('programme', function ($inscription){
             $inscription->where('departement_id', 2);
         });
         //2- On filtre les etudiants du departement IMP par programme donné
         if ($this->searchProgramme !== 0){
             $inscription->where('programme_id', $this->searchProgramme);
         }
         //3- On filtre les etudiants du departement IMP par annee_universitaire donnée
         ...
         //3- On filtre les etudiants du departement IMP par niveau donné
         if ($this->niveau !== 0) {
         ...
         IMP qui font le programme IMP et le niveau tel(ex:Licence 1)
         et la annee_universitaire (ex:2023-2024) */
        return view('livewire.departements.imp-etudiants-tables',[
